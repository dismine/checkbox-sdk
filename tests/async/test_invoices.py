...
@pytest.mark.asyncio
async def test_get_terminals(auth_token, license_key):
...
@pytest.mark.asyncio
async def test_create_invoice(auth_token, license_key, client_email):
    assert license_key, "License key is empty"
    ...
    async with AsyncCheckBoxClient() as client:
        ...
        invoice_by_id = await client.invoices.get_invoice_by_id(invoice_id=invoice["id"])
        try:
        ...
        response = await client.invoices.cancel_invoice_by_id(invoice_id=invoice["id"])
        assert response["ok"] is True

        response = await client.invoices.remove_invoice_by_id(invoice_id=invoice["id"])
        assert response["ok"] is True
...
@pytest.mark.asyncio
async def test_create_and_fiscalize_invoice(auth_token, license_key, client_email):
    assert license_key, "License key is empty"
    ...
    async with AsyncCheckBoxClient() as client:
        ...
        invoice = await client.invoices.create_and_fiscalize_invoice(
            invoice=invoice_data,
        )
        assert invoice["status"] == "CREATED"
