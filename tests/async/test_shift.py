...
async def test_close_shift_online(auth_token, license_key):
    ...
    async with AsyncCheckBoxClient(storage=storage) as client:
        ...
        with contextlib.suppress(ValueError):
            z_report = await client.shifts.close_shift_online(timeout=5, storage=storage)
            try:
...
@pytest.mark.asyncio
async def test_close_shift_by_senior_cashier(auth_token, license_key):
    assert license_key, "License key is empty"
    ...
    async with AsyncCheckBoxClient(storage=storage) as client:
        ...
        with contextlib.suppress(ValueError):
            z_report = await client.shifts.close_shift_by_senior_cashier(shift["id"], timeout=5, storage=storage)
            try:
                # sourcery skip: no-conditionals-in-tests
                if z_report:
                    model = TransactionsSchema(**z_report)
