...
async def test_add_orders(auth_token, license_key, client_email):
    ...
    async with AsyncCheckBoxClient() as client:
        ...
        try:
            ...
            for order in await client.orders.add_orders(orders_list=orders_data):
                try:
...
@pytest.mark.asyncio
async def test_get_orders(auth_token, license_key):
    assert license_key, "License key is empty"
...
@pytest.mark.asyncio
async def test_integration(auth_token, license_key):
    assert license_key, "License key is empty"
